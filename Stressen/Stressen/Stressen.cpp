...
#define INIT_ON_STACK(m_, size_) \
    ...
    m_.data = new int[size * size];//alloca(size_ * size_ * sizeof(int));
...
//void zero_1(matrix m,int size) {
//	m.x = 0;
//	m.y = 0;
//	m.size = size;
//	m.original_size = size;
//	m.data = new int[size * size];// alloca(size * size * sizeof(int));
...
int main()
{
	...
	int n[4] = { 6,8,4,2 };
	int O[4] = { 0,0,0,0 };
	matrix A = create_matrix(2, m);
	matrix B = create_matrix(2, n);
	matrix C = create_matrix(2, O);
	strassen(C, A, B);
	print_matrix(C);
